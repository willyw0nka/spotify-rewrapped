...
class SpotifyRewrappedGUI(Frame):
    ...
    def launch(self):
        ...
        messagebox.showinfo(title='Spotify rewrapped', message='Success!\nYou can check your results at {}'.format(self.output_file))
    ...
    def initUI(self):
        ...
        current_row = 0
        Label(self, text='Spotify rewrapped').grid(row=current_row, columnspan=4, sticky=N)
        ...
        Button(self, text='...', command=self.set_input_path).grid(row=current_row, column=3)
        ...
        Button(self, text='...', command=self.set_output_path).grid(row=current_row, column=3)

        current_row = 5
        Button(self, text='Generate', command=self.launch).grid(row=current_row, column=0, columnspan=2, sticky=E+W)
        Button(self, text='Quit', command=self.root.destroy).grid(row=current_row, column=2, columnspan=2, sticky=E+W)
