...
@click.command()
@click.option('--path', default='./', help='Path containing your StreamingHistory.json')
@click.option('--output', default='./spotify-unwrapped.png', help='Name of the output png')
def main(path, output):
    """Spotify rewrapped"""
    
    # Configure matplotlib
    pg = PlotGenerator(path=path, style='./resources/spotify.mplstyle', font='./resources/gotham-medium.otf')
    dm = DataManager(glob.glob(f'{path}/StreamingHistory[0-9].json'))
    ...
    # Cleanup
    os.remove(f'{path}/top-artists.png')
    os.remove(f'{path}/top-20-pie.png')
    os.remove(f'{path}/hourly-plot.png')
    os.remove(f'{path}/day-of-the-week-plot.png')
    os.remove(f'{path}/artists-through-the-year.png')
